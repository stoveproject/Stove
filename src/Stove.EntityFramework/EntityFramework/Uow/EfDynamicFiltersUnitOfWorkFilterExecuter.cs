...
namespace Stove.EntityFramework.Uow
{
    public class EfDynamicFiltersUnitOfWorkFilterExecuter : IEfUnitOfWorkFilterExecuter
    {
        ...
        {
            foreach (DbContext activeDbContext in unitOfWork.As<EfUnitOfWork>().GetAllActiveDbContexts())
            {
        ...
        {
            foreach (DataFilterConfiguration filter in unitOfWork.Filters)
            {
                if (filter.IsEnabled)
                {
                ...
                foreach (KeyValuePair<string, object> filterParameter in filter.FilterParameters)
                {
