...
{
	public static class EfCoreRepositoryRegistrar
	{
		public static void RegisterRepositories(Type dbContextType, IIocBuilder builder)
		{
			AutoRepositoryTypesAttribute autoRepositoryAttr = dbContextType.GetTypeInfo().GetSingleAttributeOrNull<AutoRepositoryTypesAttribute>() ??
			                                                  EfCoreAutoRepositoryTypes.Default;

			foreach (EntityTypeInfo entityTypeInfo in EfCoreDbContextEntityFinder.GetEntityTypeInfos(dbContextType))
			{
				Type implType;
				...
				if (primaryKeyType == typeof(int))
				{
					...
					builder.RegisterServices(r => r.UseBuilder(cb => cb.RegisterType(implType).As(genericRepositoryType).AsSelf().AsImplementedInterfaces().WithPropertyInjection()));
				}
				else
				{
					...
					builder.RegisterServices(r => r.UseBuilder(cb => cb.RegisterType(implType).As(genericRepositoryTypeWithPrimaryKey).AsSelf().AsImplementedInterfaces().WithPropertyInjection()));
				}
			}
		}
	}
